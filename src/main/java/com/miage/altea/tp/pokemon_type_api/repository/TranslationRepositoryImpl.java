...
public class TranslationRepositoryImpl implements TranslationRepository {
    ...
    public String getPokemonName(int id, Locale locale) {
            if(!(translations.containsKey(locale))){
                return translations.get(Locale.ENGLISH).stream().filter(Translation -> Translation.getId() == id)
                        .findFirst().get().getName();
            }
            return translations.get(locale).stream().filter(Translation -> Translation.getId() == id)
                    .findFirst().get().getName();
